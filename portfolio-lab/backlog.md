...
- [ ] **3.4. Refinement**:
    ...
    - [ ] Налаштувати адаптивний рендеринг у `page.tsx` (поділ на вкладки через `currentView` на мобільних пристроях / загальний вигляд на десктопі).
    - [ ] Додати плавність появи (transitions).
...
- [ ] Налаштувати валідацію форми (мінімальна довжина повідомлення).

### Layer 3: Polish (UX та Оптимізація)
- [ ] Впровадити `useOptimistic` (щоб повідомлення з'являлося в списку ще до відповіді сервера).
- [ ] Застосувати стилістику "Laboratory Entry" (моноширинний шрифт, затухання старих логів).
- [ ] Додати `revalidatePath`, щоб дані оновлювались скрізь.

---

## 📅 NEXT
- [ ] Pulse Dashboard — інтерактивне відображення статусу в реальному часі.
- [ ] Admin Zone — захищений інтерфейс для модерації логів.
